...
function readStoreConfig(options = {}) {
  ...
  if (!areHalOperatorStoreGatesOpen(envSource, options)) {
...
export function areHalOperatorStoreGatesOpen(overrides = {}, options = {}) {
  const killSwitch = overrides[OPERATOR_KILL_SWITCH_ENV] ?? env(OPERATOR_KILL_SWITCH_ENV, overrides);
  return areEmailMemoryStoreGatesOpen(overrides, { ...options, agentSlug: HAL_AGENT_SLUG }) && killSwitch !== "true";
}
...
function buildPostSessionBrief({ transcript, memoryRecord, communicationsResult, agentMailSendResult }) {
  ...
  const executiveArtifact = communicationsResult?.executive_artifact ?? null;
  const signals = [
    ...collectSignals(transcript),
    ...(executiveArtifact?.artifact_requested ? ["executive_artifact_signal"] : []),
  ].slice(0, 8);
  ...
  return {
    ...
    tl_dr: executiveArtifact?.artifact_requested
      ? `Hal captured the session, prepared a ${executiveArtifact.artifact_label}, updated safe memory, and ${sendCount > 0 ? `sent ${sendCount} AgentMail follow-up email${sendCount === 1 ? "" : "s"} with receipts` : `prepared ${draftCount} reviewable follow-up item${draftCount === 1 ? "" : "s"}`} without claiming unverified external actions.`
      : sendCount > 0
    ...
    signals,
    executive_artifact: executiveArtifact?.artifact_requested
      ? {
        artifact_id: executiveArtifact.artifact_id,
        artifact_type: executiveArtifact.artifact_type,
        artifact_label: executiveArtifact.artifact_label,
        title: executiveArtifact.title,
        summary: executiveArtifact.summary,
        section_count: safeArray(executiveArtifact.sections).length,
        next_moves: safeArray(executiveArtifact.next_moves).slice(0, 5),
      }
      : null,
    recommended_human_next_step: signals.includes("approval_boundary_signal")
      ...
      : executiveArtifact?.artifact_requested
        ? "Review the executive artifact, then decide which next move should become a confirmed action."
...
export async function storeHalMeetingPrepBrief(input = {}, options = {}) {
  ...
  if (!areHalOperatorStoreGatesOpen(envSource, options) || input.persist !== true) {
    return {
      ...brief,
      stored: false,
