...
const DANI_EMAIL_IMAGE_URL = `${EMAIL_ASSET_BASE_URL}/agents/thumbnails/dani-tavus-custom-replica.png`;
...
function emailOuter(content, options = {}) {
  ...
  return `
    <div style="margin:0;padding:0;background:${background};font-family:Inter,Segoe UI,Arial,Helvetica,sans-serif;color:#111827;">
...
function emailHeader({ title, subtitle, badge, dark = true }) {
  const heroBackground = dark
    ? "linear-gradient(135deg,#07122f 0%,#111557 48%,#5b21b6 100%)"
    : "linear-gradient(135deg,#f8fbff 0%,#ede9fe 52%,#e0f2fe 100%)";
  const titleColor = dark ? "#ffffff" : "#111827";
  const subtitleColor = dark ? "#dbeafe" : "#475569";
  return `
    <tr>
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
          <tr>
            <td style="padding:26px 24px 22px 24px;vertical-align:top;">
              <div style="font-size:13px;font-weight:900;letter-spacing:.22em;color:${dark ? "#a5b4fc" : "#4f46e5"};text-transform:uppercase;">X Agents</div>
              <h1 style="margin:16px 0 8px 0;font-size:28px;line-height:1.05;color:${titleColor};font-weight:900;">${escapeHtml(title)}</h1>
            ...
            <td align="right" style="width:150px;padding:20px 24px 20px 0;vertical-align:top;">
              <img src="${DANI_EMAIL_IMAGE_URL}" width="96" height="96" alt="Dani" style="display:block;width:96px;height:96px;border-radius:50%;object-fit:cover;border:2px solid rgba(255,255,255,.72);box-shadow:0 14px 34px rgba(37,99,235,.35);" />
              ${badge ? `<div style="margin-top:10px;padding:9px 10px;border-radius:12px;background:${dark ? "rgba(15,23,42,.78)" : "#ffffff"};color:${dark ? "#ffffff" : "#111827"};font-size:12px;line-height:1.2;font-weight:900;text-align:center;box-shadow:0 10px 24px rgba(15,23,42,.18);">${badge}</div>` : ""}
            </td>
          </tr>
        </table>
      </td>
...
function buildUserFollowupHtml(insights, schedulingContext) {
  const calendlyText = buildCalendlyCta(schedulingContext, insights);
  ...
  return emailOuter(`
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;max-width:680px;border-collapse:separate;border-spacing:0;background:#ffffff;border:1px solid #e8ecf7;border-radius:14px;overflow:hidden;box-shadow:0 18px 50px rgba(15,23,42,.12);">
      ${emailHeader({
        title: "Thanks for speaking with Dani.",
        subtitle: "Here are the follow-up details and next steps captured from your session.",
        badge: "30 min demo",
        dark: true,
      })}
      <tr>
        <td style="padding:26px 28px 8px 28px;">
          <p style="margin:0 0 18px 0;font-size:14px;color:#4f46e5;font-weight:800;">Hi ${escapeHtml(greetingName)},</p>
          <p style="margin:0 0 20px 0;font-size:15px;line-height:1.6;color:#334155;">Thank you for spending time with Dani. We captured your request and prepared the follow-up for review.</p>
          <div style="border:1px solid #e6e9f4;border-radius:12px;padding:18px 20px;background:#fbfcff;">
            <h2 style="margin:0 0 12px 0;font-size:16px;color:#111827;">Discussion Summary</h2>
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
              ${renderEmailRows([
                { label: "Business / Context", value: formatEmailValue(insights.company) },
                { label: "Requested Next Step", value: insights.requestedAction },
                { label: "Requested Meeting Time", value: meetingTimeDisplay },
                { label: "Primary Topic", value: insights.focus },
      ...
      <tr>
        <td style="padding:16px 28px 8px 28px;">
          <div style="border-radius:12px;padding:18px 20px;background:linear-gradient(135deg,#060b22 0%,#15164e 60%,#6d28d9 100%);color:#ffffff;">
            <h2 style="margin:0 0 8px 0;font-size:16px;color:#ffffff;">Scheduling / Confirmation</h2>
            <p style="margin:0;font-size:14px;line-height:1.55;color:#e0e7ff;">${insights.meetingTime ? `You mentioned this meeting time: <strong>${escapeHtml(insights.meetingTime)}</strong>.` : "No specific meeting time was captured."}</p>
          </div>
        </td>
      </tr>
      ${calendlyLink ? `
      <tr>
        <td style="padding:8px 28px 16px 28px;">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;border:1px solid #ddd6fe;border-radius:12px;background:#f7f3ff;">
            <tr>
              <td style="padding:16px 18px;">
                <div style="font-size:15px;font-weight:900;color:#20113f;margin-bottom:5px;">Let's lock in your Dani Demo Call</div>
                <div style="font-size:12px;line-height:1.45;color:#5b5470;">Choose the closest available 30-minute time that works best for you.</div>
              </td>
              <td align="right" style="padding:16px 18px;width:210px;">
                <a href="${escapeHtml(calendlyLink)}" style="display:inline-block;background:#5b21e8;color:#ffffff;text-decoration:none;border-radius:8px;padding:12px 16px;font-size:13px;font-weight:900;">Choose your time &#8594;</a>
              </td>
            </tr>
          </table>
        </td>
      ...
      <tr>
        <td style="padding:0 28px 26px 28px;">
          <div style="border:1px solid #e5e7eb;border-radius:10px;background:#ffffff;padding:14px 16px;color:#475569;font-size:13px;line-height:1.55;">
            <strong style="color:#111827;">Please note:</strong> a meeting is not considered scheduled until you confirm a time or the AI Fusion Labs team confirms availability.
          </div>
          <p style="margin:18px 0 0 0;font-size:14px;line-height:1.55;color:#334155;">${escapeHtml(formatEmailValue(insights.company, "AI Fusion Labs")) ? `Looking forward to helping ${escapeHtml(formatEmailValue(insights.company, "your team"))} move forward with clarity.` : "Looking forward to helping you move forward with clarity."}</p>
          <table role="presentation" cellspacing="0" cellpadding="0" style="margin-top:18px;border-collapse:collapse;">
            <tr>
              <td style="padding-right:10px;"><img src="${DANI_EMAIL_IMAGE_URL}" width="42" height="42" alt="Dani" style="display:block;width:42px;height:42px;border-radius:50%;object-fit:cover;border:1px solid #ddd6fe;" /></td>
              <td style="font-size:13px;color:#475569;line-height:1.35;"><strong style="display:block;color:#111827;">Dani</strong>AI Concierge, X Agents</td>
            </tr>
          </table>
        </td>
      </tr>
...
function buildAdminSummaryHtml(insights, providerConversationId, transcriptSignals, schedulingContext) {
  const meetingTimeDisplay = insights.meetingTime
    ? `${insights.meetingTime}${insights.meetingTimeSource === "prior_memory" ? " (from prior notes)" : ""}`
    : "Not specified";
  return emailOuter(`
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;max-width:660px;border-collapse:separate;border-spacing:0;background:#0f1b32;border:1px solid #2a3a59;border-radius:14px;overflow:hidden;box-shadow:0 20px 54px rgba(15,23,42,.22);">
      ${emailHeader({
        title: "New Dani Intake",
        subtitle: "Operator action required. A new intake has been captured and requires review.",
        badge: "Review",
        dark: true,
      })}
      <tr>
        <td style="padding:22px 24px 24px 24px;background:#111d35;">
          ${[
            {
              number: "1",
            ...
            },
            {
              ...
              checklist: [
                "Review the transcript and intake context.",
                "Confirm whether a meeting invite or direct follow-up is needed.",
                "Verify availability before sending any calendar commitment.",
              ],
            },
          ].map((section) => `
            <div style="margin:0 0 12px 0;border:1px solid #293b5d;border-radius:12px;background:#14213a;padding:16px;">
              <h2 style="margin:0 0 12px 0;color:#f8fafc;font-size:16px;line-height:1.2;"><span style="display:inline-block;width:22px;height:22px;border-radius:50%;background:#7c3aed;color:#ffffff;text-align:center;line-height:22px;font-size:12px;margin-right:8px;">${section.number}</span>${escapeHtml(section.title)}</h2>
              ${section.rows ? `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">${renderEmailRows(section.rows, { labelColor: "#9fb2d8", valueColor: "#ffffff", borderColor: "#293b5d" })}</table>` : ""}
              ${section.checklist ? `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">${renderChecklist(section.checklist, { color: "#dbeafe", checkColor: "#7dd3fc" })}</table>` : ""}
            </div>
          `).join("")}
          <a href="https://x-agent-website-t.vercel.app/admin/hermes-email-memory-preview" style="display:block;margin-top:18px;background:linear-gradient(90deg,#7c3aed,#2563eb);color:#ffffff;text-align:center;text-decoration:none;border-radius:8px;padding:15px 18px;font-size:14px;font-weight:900;">Review Intake Details &#8594;</a>
        </td>
      </tr>
    </table>
  `, { background: "#eef3ff" });
...
function buildLeadIntelHtml(insights, transcriptSignals, schedulingContext) {
  const meeting = insights.meetingTime ?? "Not specified";
  return emailOuter(`
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;max-width:690px;border-collapse:separate;border-spacing:0;background:#ffffff;border:1px solid #dfe6f6;border-radius:14px;overflow:hidden;box-shadow:0 20px 52px rgba(15,23,42,.14);">
      ${emailHeader({
        title: "Dani Lead Intelligence Report",
        subtitle: "AI-powered insights. Human-ready next steps.",
        badge: `Score<br><span style="font-size:28px;line-height:1;">${insights.leadScore}/10</span>`,
        dark: true,
      })}
      <tr>
        <td style="padding:22px 28px 8px 28px;">
          <p style="margin:0 0 18px 0;font-size:14px;line-height:1.55;color:#334155;">Hi Team,<br />Here is the post-session intelligence snapshot for Dani. High-intent signals and recommended next steps are ready for review.</p>
          ${[
            {
              title: "1. Contact / Account",
              rows: [
                { label: "Lead temperature", value: insights.returningVisitor ? "Returning warm lead" : "New / unknown lead" },
                { label: "Account context", value: formatEmailValue(insights.company) },
                { label: "Visitor name heard", value: formatEmailValue(insights.visitorName) },
              ],
            },
            {
              title: "2. Opportunity Signals",
              rows: [
            ...
            <div style="margin:0 0 14px 0;border:1px solid #e7ecf7;border-radius:12px;background:#fbfcff;padding:16px 18px;">
              <h2 style="margin:0 0 12px 0;color:#111827;font-size:15px;line-height:1.25;text-transform:uppercase;letter-spacing:.04em;">${escapeHtml(section.title)}</h2>
              ${section.rows ? `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">${renderEmailRows(section.rows)}</table>` : ""}
              ${section.checklist ? `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">${renderChecklist(section.checklist)}</table>` : ""}
            </div>
          `).join("")}
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:18px 0;border-collapse:collapse;background:#07122f;border-radius:12px;overflow:hidden;">
            <tr>
              ${[
                ["Prospect Score", `${insights.leadScore}/10`],
                ["Intent Level", insights.meetingRequested ? "High" : "Moderate"],
                ...
                <td align="center" style="padding:16px 8px;border-right:1px solid rgba(255,255,255,.12);">
                  <div style="font-size:12px;color:#93c5fd;text-transform:uppercase;letter-spacing:.06em;">${escapeHtml(label)}</div>
                  <div style="margin-top:6px;font-size:15px;color:#ffffff;font-weight:900;">${escapeHtml(value)}</div>
                </td>
              `).join("")}
            </tr>
          </table>
          <a href="https://x-agent-website-t.vercel.app/admin/hermes-email-memory-preview" style="display:block;margin:0 auto 14px auto;max-width:360px;background:linear-gradient(90deg,#7c3aed,#2563eb);color:#ffffff;text-align:center;text-decoration:none;border-radius:8px;padding:14px 18px;font-size:14px;font-weight:900;">Open Opportunity Brief &#8594;</a>
          <p style="margin:0 0 22px 0;text-align:center;font-size:12px;color:#64748b;">AI insights. Human conversations. Better outcomes.</p>
        </td>
      </tr>
    </table>
  `, { background: "#f5f7fc" });
...
function buildDesignedEmailHtmlPreview({
  ...
  if (actionType === "email.user_followup") {
    return buildUserFollowupHtml(insights, schedulingContext, bodyText);
  }
  if (actionType === "email.admin_summary") {
    return buildAdminSummaryHtml(insights, providerConversationId, transcriptSignals, schedulingContext);
  }
  if (actionType === "email.lead_intel") {
    return buildLeadIntelHtml(insights, transcriptSignals, schedulingContext);
  }
...
function buildVisitorFollowupEmail(insights, schedulingContext) {
  ...
  return [
    ...
    insights.meetingTimeSource === "prior_memory"
      ? "Thank you for speaking with Dani again. We connected this session to your prior notes and prepared the follow-up request for review."
      : insights.meetingTime
      ? "Thank you for speaking with Dani. We captured your requested meeting window and the follow-up items you asked us to include."
