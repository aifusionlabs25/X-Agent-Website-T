...
import { buildHalExecutiveArtifact } from "./halExecutiveArtifactComposer.mjs";
...
const MAX_EMAIL_BODY_LENGTH = 5200;
...
function hasExecutiveArtifact(executiveArtifact) {
  return Boolean(executiveArtifact?.artifact_requested && Array.isArray(executiveArtifact.sections));
}

function buildExecutiveArtifactTextSection(executiveArtifact) {
  if (!hasExecutiveArtifact(executiveArtifact)) return [];
  return [
    "",
    "Executive Artifact",
    executiveArtifact.title,
    executiveArtifact.summary,
    "",
    ...executiveArtifact.sections.flatMap((section) => [
      section.heading,
      bullet(section.items),
      "",
    ]),
    "Execution Boundary",
    "This is a prepared executive artifact, not proof that Hal completed an external action. Email, calendar, CRM, meeting, or approval actions still require a tool receipt or human confirmation.",
  ];
}

function renderExecutiveArtifactHtml(executiveArtifact, profile = getXAgentProfile(DANI_AGENT_SLUG)) {
  if (!hasExecutiveArtifact(executiveArtifact)) return "";
  const theme = emailTheme(profile);
  return `
    <tr>
      <td style="padding:16px 28px 8px 28px;">
        <div style="border:1px solid ${theme.border};border-radius:14px;background:${theme.canvasAlt};overflow:hidden;">
          <div style="padding:18px 20px;background:${theme.darkPanel};border-bottom:1px solid ${theme.darkBorder};">
            <div style="font-size:11px;font-weight:900;letter-spacing:.18em;color:${theme.eyebrow};text-transform:uppercase;">Executive Artifact</div>
            <h2 style="margin:8px 0 7px 0;color:${theme.headerText};font-size:22px;line-height:1.12;font-family:Georgia,'Times New Roman',serif;">${escapeHtml(executiveArtifact.title)}</h2>
            <p style="margin:0;color:${theme.headerMuted};font-size:13px;line-height:1.55;">${escapeHtml(executiveArtifact.summary)}</p>
          </div>
          <div style="padding:18px 20px;">
            ${executiveArtifact.sections.map((section) => `
              <div style="margin:0 0 16px 0;">
                <h3 style="margin:0 0 9px 0;color:${theme.text};font-size:14px;line-height:1.25;text-transform:uppercase;letter-spacing:.05em;">${escapeHtml(section.heading)}</h3>
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
                  ${renderChecklist(section.items, { color: theme.muted, checkColor: theme.accentDark })}
                </table>
              </div>
            `).join("")}
            <div style="border-top:1px solid ${theme.border};padding-top:13px;color:${theme.muted};font-size:12px;line-height:1.55;">
              <strong style="color:${theme.text};">Boundary:</strong> This is prepared synthesis, not proof that Hal completed an external action. Tool receipts or human approval are still required before any completion claim.
            </div>
          </div>
        </div>
      </td>
...
function buildUserFollowupHtml(
  insights,
  schedulingContext,
  ...
  return emailOuter(`
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;max-width:680px;border-collapse:separate;border-spacing:0;background:${theme.canvas};border:1px solid ${theme.border};border-radius:14px;overflow:hidden;box-shadow:${theme.shadow};">
      ...
      ${renderExecutiveArtifactHtml(executiveArtifact, profile)}
      <tr>
        <td style="padding:16px 28px 8px 28px;">
...
function buildDesignedEmailHtmlPreview({
  ...
  if (actionType === "email.user_followup") {
    return buildUserFollowupHtml(insights, schedulingContext, profile, executiveArtifact);
  }
...
function buildVisitorFollowupEmail(
  insights,
  schedulingContext,
  profile = getXAgentProfile(DANI_AGENT_SLUG),
  executiveArtifact = null,
  ...
  if (isHalProfile(profile)) {
    return [
      ...
      ...buildExecutiveArtifactTextSection(executiveArtifact),
      "",
      "Human Handoff / Next Steps",
      bullet([
...
export function buildHermesEmailCommunicationPlan({
  ...
  const executiveArtifact = isHalProfile(profile)
  ...
  const userFollowup = buildDraftAction({
    ...
    bodyText: buildVisitorFollowupEmail(emailInsights, schedulingContext, profile, executiveArtifact),
  ...
  const leadIntel = buildDraftAction({
    ...
    bodyText: buildLeadIntelEmail(emailInsights, transcriptSignals, schedulingContext, profile, executiveArtifact),
    insights: emailInsights,
    schedulingContext,
    transcriptSignals,
    transcriptMetadata,
  ...
  return {
    ...
    email_insight_metadata: {
      ...
      executive_artifact_requested: Boolean(executiveArtifact?.artifact_requested),
      executive_artifact_type: executiveArtifact?.artifact_type ?? "none",
      executive_artifact_label: executiveArtifact?.artifact_label ?? "No executive artifact requested",
    },
    executive_artifact: executiveArtifact?.artifact_requested ? executiveArtifact : null,
