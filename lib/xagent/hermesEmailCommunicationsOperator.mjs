...
function readCalendlyUrl(overrides = {}) {
  const value = env(CALENDLY_URL_ENV, overrides);
  if (!value) return "";
  try {
...
function collectEmailInsights(transcript) {
  ...
  const meetingRequested = combined.some((turn) => (
...
function buildCalendlyCta({ calendlyUrl, meetingRequested }, insights = {}) {
  ...
  if (meetingRequested && insights.meetingTime) {
    return [
      ...
      `Use this link to choose or confirm the closest available ${CALENDLY_MEETING_LENGTH} ${CALENDLY_MEETING_LABEL}: ${calendlyUrl}`,
      "If that exact time is not open, the AI Fusion Labs team can coordinate the closest available option.",
    ].join("\n");
  }
  if (meetingRequested) {
    return `Use this link to choose a ${CALENDLY_MEETING_LENGTH} ${CALENDLY_MEETING_LABEL}: ${calendlyUrl}`;
  }
  return `If a deeper walkthrough would be helpful, you can book a ${CALENDLY_MEETING_LENGTH} ${CALENDLY_MEETING_LABEL} here: ${calendlyUrl}`;
}
...
function buildVisitorFollowupEmail(insights, schedulingContext) {
  ...
  return [
    ...
    bullet([
      ...
      `Requested meeting time: ${insights.meetingTime ?? "not specified"}`,
...
function buildLeadIntelEmail(insights, transcriptSignals, schedulingContext) {
  return [
    ...
    bullet([
      ...
      `Scheduling intent: ${insights.meetingTime ?? "not specified"}`,
      `Calendly CTA included: ${schedulingContext.calendlyCtaIncluded ? "yes" : "no"}`,
...
function buildDraftAction({
  ...
  return {
    ...
    body_text_preview: truncateMultiline(redactEmailLikeText(bodyText), MAX_EMAIL_BODY_LENGTH),
