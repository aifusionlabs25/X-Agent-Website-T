...
function buildCalendlyCta({ calendlyUrl, meetingRequested }) {
  if (!calendlyUrl) return "";
  if (meetingRequested) {
    return `To make scheduling easy, you can book a ${CALENDLY_MEETING_LENGTH} ${CALENDLY_MEETING_LABEL} here: ${calendlyUrl}`;
  }
...
function buildVisitorFollowupEmail(insights, schedulingContext) {
  ...
  const calendlyCta = buildCalendlyCta(schedulingContext);
  return [
    `Hi${greetingName},`,
    "",
    ...
    bullet([
      ...
      `Timing discussed: ${insights.meetingTime ?? "not specified"}`,
      `Primary topic: ${insights.focus}`,
    ]),
    calendlyCta ? "" : null,
    calendlyCta ? "Schedule a Time" : null,
    calendlyCta || null,
    "",
