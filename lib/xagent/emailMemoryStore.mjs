...
function resolveAgentScope(options = {}) {
  return {
    tenantId: assertString(options.tenantId ?? DANI_TENANT_ID, "tenantId"),
    agentSlug: assertString(options.agentSlug ?? DANI_AGENT_SLUG, "agentSlug"),
  };
}
...
function readAgentPilotValue(overrides = {}, options = {}, profileKey, fallbackEnv) {
  const { agentSlug } = resolveAgentScope(options);
  const profile = getXAgentProfile(agentSlug);
  const agentEnv = profile?.[profileKey];
  return (agentEnv ? overrides[agentEnv] ?? env(agentEnv, overrides) : "") || overrides[fallbackEnv] || env(fallbackEnv, overrides);
}

function readGateConfig(overrides = {}, options = {}) {
  return {
    enabled: overrides[ENABLED_ENV] ?? env(ENABLED_ENV, overrides),
    agentPilotEnabled: readAgentPilotValue(overrides, options, "emailMemoryStorePilotEnv", DANI_PILOT_ENV),
    killSwitch: overrides[KILL_SWITCH_ENV] ?? env(KILL_SWITCH_ENV, overrides),
  };
}

export function areEmailMemoryStoreGatesOpen(overrides = {}, options = {}) {
...
function readOutboundContactGateConfig(overrides = {}, options = {}) {
  return {
    enabled: overrides[OUTBOUND_CONTACT_ENABLED_ENV] ?? env(OUTBOUND_CONTACT_ENABLED_ENV, overrides),
    agentPilotEnabled: readAgentPilotValue(
      overrides,
      options,
      "emailOutboundContactStorePilotEnv",
      OUTBOUND_CONTACT_DANI_PILOT_ENV,
    ),
    killSwitch: overrides[OUTBOUND_CONTACT_KILL_SWITCH_ENV] ?? env(OUTBOUND_CONTACT_KILL_SWITCH_ENV, overrides),
  };
}

export function areEmailOutboundContactStoreGatesOpen(overrides = {}, options = {}) {
  const gates = readOutboundContactGateConfig(overrides, options);
  return (
    gates.enabled === "true"
    && gates.agentPilotEnabled === "true"
    && gates.killSwitch === "false"
...
export function deriveStoredEmailIdentityHash(email, options = {}) {
  ...
  const { tenantId, agentSlug } = resolveAgentScope(options);
  return sha256(`${identitySalt}:${tenantId}:${agentSlug}:${normalizedEmail}`);
}
...
function visitorMemoryNamespace(emailIdentityHash, options = {}) {
  const { tenantId, agentSlug } = resolveAgentScope(options);
  return `xagents/${tenantId}/${agentSlug}/${emailVisitorId(emailIdentityHash)}`;
}

function conversationMappingKey(providerConversationId, options = {}) {
  const { tenantId, agentSlug } = resolveAgentScope(options);
  return `xagent:${tenantId}:${agentSlug}:conversation:${providerConversationId}`;
}

function latestMemoryKey(emailIdentityHash, options = {}) {
  const { tenantId, agentSlug } = resolveAgentScope(options);
  return `xagent:${tenantId}:${agentSlug}:email:${emailIdentityHash}:memory:latest`;
}

function memoryHistoryKey(emailIdentityHash, options = {}) {
  const { tenantId, agentSlug } = resolveAgentScope(options);
  return `xagent:${tenantId}:${agentSlug}:email:${emailIdentityHash}:memory:history`;
}
...
export async function storeConversationEmailMappingForStart(input, options = {}) {
  ...
  if (!email || !areEmailMemoryStoreGatesOpen(options.env ?? process.env, options)) {
    return {
  ...
  const normalizedEmail = normalizeEmailIdentityInput(email);
  const emailIdentityHash = deriveStoredEmailIdentityHash(normalizedEmail, {
  ...
  const outboundContactStoreEnabled = areEmailOutboundContactStoreGatesOpen(options.env ?? process.env, options);

  const mapping = {
    schema_version: STORE_SCHEMA_VERSION,
    artifact_purpose: "xagent_conversation_email_memory_mapping",
    tenant_id: scope.tenantId,
    agent_slug: scope.agentSlug,
    ...
    visitor_memory_namespace: visitorMemoryNamespace(emailIdentityHash, scope),
  ...
  await redisSet(conversationMappingKey(providerConversationId, scope), mapping, options);
...
export async function readConversationEmailMapping(providerConversationId, options = {}) {
  ...
  return redisGet(conversationMappingKey(id, options), options);
}
...
function assertConversationEmailMapping(mapping) {
  if (!mapping || typeof mapping !== "object") {
    throw new Error("conversation email mapping is required");
  }
  assertString(mapping.tenant_id, "tenant_id");
  assertString(mapping.agent_slug, "agent_slug");
  assertString(mapping.provider_conversation_id, "provider_conversation_id");
  assertString(mapping.session_id, "session_id");
  assertString(mapping.visitor_id, "visitor_id");
  assertString(mapping.visitor_memory_namespace, "visitor_memory_namespace");
  assertString(mapping.email_identity_hash, "email_identity_hash");
  return mapping;
}
...
export function buildEmailMemoryRecordFromSummary({
  ...
  const scopedMapping = assertConversationEmailMapping(mapping);
  ...
  const recordWithoutHash = {
    ...
    tenant_id: scopedMapping.tenant_id,
    agent_slug: scopedMapping.agent_slug,
...
export function buildEmailMemoryRecordFromTranscript({ mapping, transcript, transcriptMetadata, operatorMetadata } = {}) {
  assertConversationEmailMapping(mapping);
...
export function buildSessionStartMemoryContextFromStoredRecords(records, options = {}) {
  ...
  const memoryContext = {
    ...
    allowed_use: [
      ...
      "support continuity in a new agent session",
    ],
    forbidden_use: [
  ...
  return buildSessionStartMemoryContextPreview(memoryContext, {
    tenantId: latestRecord.tenant_id,
    agentSlug: latestRecord.agent_slug,
  });
}
...
export async function storeEmailMemoryRecord(record, options = {}) {
  ...
  const existingHistory = await redisGet(memoryHistoryKey(emailIdentityHash, scope), options);
  const previousLatest = await redisGet(latestMemoryKey(emailIdentityHash, scope), options);
  const memoryHistory = mergeMemoryHistory({
    existingHistory,
    previousLatest,
    nextRecord: record,
  });
  await redisSet(latestMemoryKey(emailIdentityHash, scope), record, options);
  await redisSet(memoryHistoryKey(emailIdentityHash, scope), memoryHistory, options);
  return {
...
export async function readStoredEmailMemoryContext(input, options = {}) {
  if (!areEmailMemoryStoreGatesOpen(options.env ?? process.env, options)) {
    return null;
  }
  const config = readStoreConfig(options);
  const scope = resolveAgentScope(options);
  const emailIdentityHash = deriveStoredEmailIdentityHash(input?.email, {
    identitySalt: config.identitySalt,
    ...scope,
  });
  const record = await redisGet(latestMemoryKey(emailIdentityHash, scope), options);
  const storedHistory = await redisGet(memoryHistoryKey(emailIdentityHash, scope), options);
  const memoryHistory = mergeMemoryHistory({
...
export async function readStoredEmailMemoryRecordForConversation(providerConversationId, options = {}) {
  ...
  return redisGet(latestMemoryKey(mapping.email_identity_hash, {
    tenantId: mapping.tenant_id,
    agentSlug: mapping.agent_slug,
  }), options);
}
