...
export function summarizeTranscriptForMemory(transcript) {
  ...
  const selected = userTurns.slice(0, 6);
  const summaryBody = selected.length > 0
    ? selected.join(" ")
    : "Visitor completed a Dani session, but no durable business preference was captured safely.";

  return {
    summary: truncate(`Visitor discussed: ${summaryBody}`, 900),
    confidence: selected.length >= 3 ? 0.74 : 0.58,
