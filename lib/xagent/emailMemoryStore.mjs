...
function isEmailCollectionTurn(content) {
  const value = String(content ?? "").toLowerCase();
  return (
    value.includes("@")
    || /\b(?:email|e-mail)\b/.test(value)
    || /\b(?:gmail|yahoo|outlook|hotmail)\b/.test(value)
    || /\b[a-z](?:\s*-\s*[a-z]){2,}/i.test(value)
    || /\bat\s+\w+\s+dot\s+\w+/.test(value)
  );
}
...
function selectTurn(turns, patterns, used = new Set()) {
...
function formatMemorySummarySections(sections, fallbackTurns) {
  ...
  for (const [label, value] of sections) {
    if (!value) continue;
    lines.push(`${label}: ${truncate(value, 220)}`);
  }
