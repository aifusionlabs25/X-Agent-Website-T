...
export function buildDaniLiveNotesState(input = {}) {
  ...
  const memoryStatus = memoryApplied
    ? "Approved prior notes are active"
    : memoryRequested && !freshSession
      ? "Memory check-in received"
      : "Fresh session";

  const handoffChannel = memoryRequested && !freshSession
    ? "Website check-in identity"
    : "Not supplied yet";

  const notes = {
    title: "Dani's Notes",
    subtitle: "Safe session cues only. No private backend data is shown.",
    visitorLabel,
    statusChips: [
      { label: "Notes active", status: "active" },
    ...
    rows: [
      ...
      { label: "Memory", value: memoryStatus },
      {
        ...
        value: memoryApplied ? "Continuing from approved prior notes" : "Listening for business context",
      },
      {
        label: "Current use case",
        value: memoryApplied ? "Returning conversation" : "Discovery in progress",
      },
      {
        label: "Requested next step",
        value: "Dani will capture the next clear next step",
      },
      {
        label: "Meeting or email request",
        value: "Available for post-session handoff if requested",
      },
      { label: "Preferred handoff", value: handoffChannel },
    ],
    openQuestions: [
      "What outcome should Dani help move forward?",
      "Which team member or system should receive the handoff?",
    ],
    footer: memoryApplied
      ? "Dani can use approved prior notes, but completed actions still require backend confirmation."
