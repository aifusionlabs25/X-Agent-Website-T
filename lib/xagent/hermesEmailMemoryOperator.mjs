...
function readGateConfig(overrides = {}, options = {}) {
  ...
  const agentPilotEnv = profile.hermesMemoryOperatorPilotEnv;
  return {
    enabled: env(ENABLED_ENV, overrides),
    agentPilotEnabled: (agentPilotEnv ? env(agentPilotEnv, overrides) : "") || env(DANI_PILOT_ENV, overrides),
    killSwitch: env(KILL_SWITCH_ENV, overrides),
  };
}

export function areHermesMemoryOperatorGatesOpen(overrides = {}, options = {}) {
...
function buildOperatorRequest({ mapping, transcript, transcriptMetadata } = {}) {
  return {
    ...
    tenant_id: mapping.tenant_id ?? DANI_TENANT_ID,
    agent_slug: mapping.agent_slug ?? DANI_AGENT_SLUG,
...
export async function runHermesEmailMemoryOperator({ provider_conversation_id, transcript, transcriptMetadata } = {}, options = {}) {
  ...
  const enabled = areHermesMemoryOperatorGatesOpen(envSource, options);
