...
export function validateSessionStartMemoryContext(input, options = {}) {
  ...
  const expectedAgentSlug = assertString(options.agentSlug ?? DANI_AGENT_SLUG, "agentSlug");
  const expectedTenantId = assertString(options.tenantId ?? DANI_TENANT_ID, "tenantId");

  if (agentSlug !== expectedAgentSlug) {
    throw new Error(`agent_slug must be ${expectedAgentSlug}`);
  }
  if (tenantId !== expectedTenantId) {
    throw new Error(`tenant_id must be ${expectedTenantId}`);
  }
...
export function buildSessionStartMemoryContextPreview(input, options = {}) {
  const context = validateSessionStartMemoryContext(input, options);
...
export function buildGatedSessionStartMemoryContextPreview(input, options = {}) {
  ...
  return buildSessionStartMemoryContextPreview(input, options);
}
