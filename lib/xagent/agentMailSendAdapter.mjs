...
function buildTranscriptAttachment(providerConversationId, transcript) {
  const turns = normalizeTranscriptAttachmentTurns(transcript);
  if (turns.length === 0) return null;

  const lines = [
    "Dani X Agent Transcript",
  ...
  return {
    ...
    filename: `dani-transcript-${safeAttachmentConversationId(providerConversationId)}.txt`,
    content_type: "text/plain",
  };
}

function attachmentsForAction(actionType, input = {}, providerConversationId = "") {
  if (actionType !== "email.lead_intel") return [];
  const attachment = buildTranscriptAttachment(providerConversationId, input.transcript);
...
export async function runAgentMailPostSessionSends(input = {}, options = {}) {
  ...
  for (const action of actions) {
    ...
    const attachments = attachmentsForAction(actionType, input, providerConversationId);
