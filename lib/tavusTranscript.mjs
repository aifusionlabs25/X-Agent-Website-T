...
export function extractTranscriptDetailsFromTranscriptionReadyEvent(event) {
  if (!event || typeof event !== "object") {
    throw new Error("transcription_ready event is required");
  }
  if (event.event_type !== TRANSCRIPTION_READY_EVENT) {
    throw new Error("event_type must be application.transcription_ready");
  }

  return extractTranscriptDetailsFromTranscriptArray(event?.properties?.transcript);
}

function extractTranscriptDetailsFromTranscriptArray(sourceTranscript) {
...
export function extractTranscriptDetailsFromVerboseConversation(verboseConversation) {
  ...
  return extractTranscriptDetailsFromTranscriptionReadyEvent(event);
}
